...
var Session = /** @class */ (function () {
    ...
    Session.prototype.publish = function (publisher) {
        ...
        if (!publisher.stream.isPublisherPublished) {
            ...
            var mypublisher_1 = this.openVidu.initPublisher(publisher.stream.getParentId(), this.openVidu.openVidu.storedPublisherOptions);
            if (mypublisher_1.isScreenRequested && !mypublisher_1.stream.isScreenRequestedReady) {
                mypublisher_1.stream.addOnceEventListener('screen-ready', function () {
                    _this.streamPublish(mypublisher_1);
                });
            }
            else {
                this.streamPublish(mypublisher_1);
            }
        }
    };
                ...
                if (error) {
                    ...
                    if (_this.camera.getSendAudio()) {
                        ...
                        navigator.mediaDevices.getUserMedia({ audio: true, video: false })
                            ...
                            _this.camera.getMediaStream().addTrack(userStream.getAudioTracks()[0]);
